...
- (void)onUserleft:(unsigned int)userID
{
    ...
    if(displayThumbnailVideoView)
    {
        ...
        [displayThumbnailVideoView release];
...
- (void)initArrowButton
{
     ...
     if(!_downArrowButton)
     {
        _downArrowButton = [[[NSButton alloc] initWithFrame:NSMakeRect((self.frame.size.width - _upArrowButton.frame.size.width)/2, marigin, 30, 30)] autorelease];
     ...
     if(!_upArrowButton)
     {
         _upArrowButton = [[[NSButton alloc] initWithFrame:NSMakeRect((self.frame.size.width - _upArrowButton.frame.size.width)/2, self.frame.size.height - marigin - 30, 30, 30)] autorelease];
